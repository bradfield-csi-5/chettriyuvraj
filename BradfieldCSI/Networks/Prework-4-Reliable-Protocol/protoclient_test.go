...
func TestYDPPacket(t *testing.T) {
	t.Run("test addr to byte slice", func(t *testing.T) {
		got, _ := AddrToByteSlice(unix.SockaddrInet4{Port: 64403, Addr: [4]byte{0x7F, 0x00, 0x00, 0x01}})
	...
	t.Run("test YDP packet message", func(t *testing.T) {
		got, _ := NewYDPPacket([]byte("Hi guys"), 0x01, RECVSOCKADDR, unix.SockaddrInet4{Port: 64403, Addr: [4]byte{0x7F, 0x00, 0x00, 0x01}})
	...
	t.Run("test YDP packet encoding to Binary", func(t *testing.T) {
		packet, _ := SampleYDPPacket([]byte("Hi guys"), 0x01, RECVSOCKADDR, unix.SockaddrInet4{Port: 64403, Addr: [4]byte{0x7F, 0x00, 0x00, 0x01}}, 0x65157a3a)
		got, _ := packet.encode()
