...
int main (int argc, char** argv) {
  ...
  printf("%.2fs to run %d tests (%.2fns per test) test - baseline\n", time_elapsed, TEST_LOOPS,
         time_elapsed * 1e9 / TEST_LOOPS);

  clocks_elapsed = test_end - test_start;
  time_elapsed = clocks_elapsed/CLOCKS_PER_SEC;

    printf("%.2fs to run %d tests (%.2fns per test) test\n", time_elapsed, TEST_LOOPS,
         time_elapsed * 1e9 / TEST_LOOPS);

  clocks_elapsed = baseline_end - baseline_start;
  time_elapsed = clocks_elapsed/CLOCKS_PER_SEC;

  printf("%.2fs to run %d baseline tests (%.2fns per test) baseline\n", time_elapsed, TEST_LOOPS,
         time_elapsed * 1e9 / TEST_LOOPS);


  clocks_elapsed = baseline_end - baseline_start - (test_end - test_start);
  time_elapsed = clocks_elapsed/CLOCKS_PER_SEC;

  printf("Speedup: %.2fs to run %d tests (%.2fns per test) baseline-test\n", time_elapsed, TEST_LOOPS,
   time_elapsed * 1e9 / TEST_LOOPS);
  return ignore;
}